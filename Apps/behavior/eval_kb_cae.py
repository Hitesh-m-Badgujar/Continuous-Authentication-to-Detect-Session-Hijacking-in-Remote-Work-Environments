...
"""
Offline evaluation for the keyboard CAE model (CMU-derived windows).

- Loads model + scaler from Models/cae_kb
- Loads feature windows from Data/kb_cmu_windows.csv
- Uses 18-D feature vector (same as training)
- For each user:
    * samples balanced genuine + impostor windows
    * computes reconstruction residuals
    * sweeps thresholds to find tau that MAXIMISES ACC
    * computes FAR / FRR / ACC
- Writes:
    - artifacts/kb_cae_metrics.csv
    - artifacts/kb_cae_overview.json
    - artifacts/kb_cae_scores.csv  (NEW: label + score for confusion/PR)
"""

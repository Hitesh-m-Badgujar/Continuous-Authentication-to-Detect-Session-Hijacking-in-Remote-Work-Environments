...
def main() -> None:
    ...
    svm = SVC(
        ...
        decision_function_shape="ovr",
        class_weight="balanced",
        probability=True,              # <--- keep probability enabled
    )
    print("[INFO] Training final SVM with best hyperparams...")
    svm.fit(X_train_scaled, y_train)
    print("[INFO] Final SVM training done")

    # ---------------- Evaluate on test split (per-user FAR/FRR/ACC) -------------
    y_pred = svm.predict(X_test_scaled)
    users = sorted(df["user_id"].unique())
    rows = []

    # Also gather verification-style scores for confusion/PR
    proba_test = svm.predict_proba(X_test_scaled)   # shape: (n_samples, n_classes)
    classes = svm.classes_                          # list of user_ids in same order
    ...
    for uid in users:
        ...
        # ----- build binary scores for this "claimed user" uid -----
        idx = np.where(classes == uid)[0]
        if idx.size == 0:
            continue
        ci = int(idx[0])
        ...
        scores_g = proba_test[is_genuine, ci]
        scores_i = proba_test[is_impostor, ci]

        # genuine label = 1, impostor = 0
        all_scores.extend(scores_g.tolist())
        all_labels.extend([1] * len(scores_g))
        all_users.extend([uid] * len(scores_g))

        all_scores.extend(scores_i.tolist())
        all_labels.extend([0] * len(scores_i))
        all_users.extend([uid] * len(scores_i))
    ...
    scores_csv = ARTIFACTS_DIR / "kb_svm_scores.csv"
    ...
    # ---- save score-level CSV for confusion matrix + PR curve ----
    scores_df = pd.DataFrame(
        {
            "label": all_labels,     # 1 = genuine, 0 = impostor
            "score": all_scores,     # probability that uid is the user
            "user_id": all_users,    # claimed user for this trial
        }
    )
    ...
    overview = {
        ...
        "features": FEATURES,
        "model": "SVM (RBF, tuned; class_weight=balanced, probability=True)",
    ...
    print(f"WROTE SVM score CSV   -> {scores_csv}")
